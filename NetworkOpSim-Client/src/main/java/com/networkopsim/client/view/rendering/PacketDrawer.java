// FILE: /NetworkOpSim-Multiplayer/NetworkOpSim-Client/src/main/java/com/networkopsim/client/view/rendering/PacketDrawer.java
// ================================================================================
...
public final class PacketDrawer {
    ...
    private static final Random visualRandom = new Random(12345);

    private PacketDrawer() {}

    public static void drawPacketBody(Graphics2D g2d, PacketDTO packet, int drawX, int drawY, int drawSize, Color baseColor) {
        ...
        if (packetType == NetworkEnums.PacketType.MESSENGER && shape == NetworkEnums.PacketShape.CIRCLE) {
            Color messengerColor = baseColor;
            if (packet.getBulkParentId() != -1) {
                float hue = ((packet.getBulkParentId() * 31) + 17) % 100 / 100.0f;
                Color groupColor = Color.getHSBColor(hue, 0.9f, 0.95f);
                messengerColor = new Color(
                        (baseColor.getRed() + groupColor.getRed())/2,
                        (baseColor.getGreen() + groupColor.getGreen())/2,
                        (baseColor.getBlue() + groupColor.getBlue())/2
                );
            }
            ...
            switch (packetType) {
                ...
                case SECRET:
                    ...
                    break;
                case BULK:
                    drawBulkBody(g2d, drawSize, baseColor);
                    break;
                case WOBBLE:
                    drawWobbleBody(g2d, drawSize, baseColor);
                    break;
                default: // NORMAL, TROJAN
                    drawDefaultBody(g2d, shape, drawX, drawY, drawSize, halfSize, baseColor);
                    break;
            }
        }
    }

    public static void drawPacketOverlays(Graphics2D g2d, PacketDTO packet, int drawX, int drawY, int drawSize, int halfSize) {
    ...
    // <<<--- MODIFIER CHANGED TO PUBLIC STATIC ---<<<
    public static Color getColorFromPacketShape(NetworkEnums.PacketShape shape) {
    ...
    private static void drawDefaultBody(Graphics2D g2d, NetworkEnums.PacketShape shape, int drawX, int drawY, int drawSize, int halfSize, Color packetColor) {
    ...
    private static void drawSecretBody(Graphics2D g2d, boolean isUpgraded, int drawX, int drawY, int drawSize) {
        ...
        Shape oldClip = g2d.getClip(); g2d.setClip(new java.awt.geom.Ellipse2D.Double(drawX, drawY, drawSize, drawSize)); int pixelSize = Math.max(2, drawSize / 6);
        visualRandom.setSeed(drawSize * 31 + (isUpgraded ? 1 : 0));
        for (int py = drawY; py < drawY + drawSize; py += pixelSize) { for (int px = drawX; px < drawX + drawSize; px += pixelSize) { g2d.setColor(selectedPalette[visualRandom.nextInt(selectedPalette.length)]); g2d.fillRect(px, py, pixelSize, pixelSize); } }
    ...
    private static void drawBulkBody(Graphics2D g2d, int drawSize, Color baseColor) {
        ...
        for (Point2D.Double centerOffset : centers) { Color fillColor = (centerOffset.y < -v_offset * 1.5) ? dark : (centerOffset.y > v_offset * 1.5) ? light : mid; Path2D hexagonPath = createHexagon(centerOffset.x, centerOffset.y, radius); g2d.setColor(fillColor); g2d.fill(hexagonPath); g2d.setColor(outlineColor); g2d.draw(hexagonPath); }
    }
    ...
    private static void drawWobbleBody(Graphics2D g2d, int drawSize, Color baseColor) {
        Color darkNodeColor = baseColor.darker();
        Color midNodeColor = baseColor;
        Color lightNodeColor = baseColor.brighter();
        Color lightLineColor = new Color(lightNodeColor.getRed(), lightNodeColor.getGreen(), lightNodeColor.getBlue(), 150);
