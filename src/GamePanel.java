...
public class GamePanel extends JPanel {
    ...
    public GamePanel(NetworkGame game) {
        ...
        setBackground(BACKGROUND_COLOR); setPreferredSize(new Dimension(NetworkGame.WINDOW_WIDTH, NetworkGame.WINDOW_HEIGHT)); setFocusable(true);
        addKeyListener(gameInputHandler); addMouseListener(gameInputHandler); addMouseMotionListener(gameInputHandler);
    ...
    private void gameTick() {
        ...
        if (!airyamanActive) {
            detectAndHandleCollisionsSingleEvent(currentPacketsSnapshot);
        }
        processPacketBuffers(); 
        checkEndConditions();
        repaint();
    ...
    private void detectAndHandleCollisionsSingleEvent(List<Packet> packetSnapshot) {
        ...
        for (int i = 0; i < packetSnapshot.size(); i++) {
            ...
            for (int j = i + 1; j < packetSnapshot.size(); j++) {
                ...
                if (p1.collidesWith(p2)) {
                    Pair<Integer, Integer> collisionPair = makeOrderedPair(p1.getId(), p2.getId());
                    currentTickCollisions.add(collisionPair);
                    if (!activelyCollidingPairs.contains(collisionPair)) {
                        ...
                        if (!atarActive) {
                            ...
                            if (impactCenter != null) {
                                handleImpactWaveNoise(impactCenter, packetSnapshot, p1, p2);
                            }
                        }
                    }
                }
        ...
        Set<Pair<Integer, Integer>> pairsToRemove = new HashSet<>(activelyCollidingPairs);
        pairsToRemove.removeAll(currentTickCollisions); 
        activelyCollidingPairs.removeAll(pairsToRemove);
    ...
    public boolean attemptWireCreation(Port startPort, Port endPort) { if (simulationStarted) { java.lang.System.err.println("Wire creation failed: Cannot create wires during simulation."); return false; } if (startPort == null || endPort == null || startPort.getPosition() == null || endPort.getPosition() == null) { java.lang.System.err.println("Wire creation failed: Start or end port is null or has no position."); return false; } if (startPort.getType() != NetworkEnums.PortType.OUTPUT || endPort.getType() != NetworkEnums.PortType.INPUT || startPort.isConnected() || endPort.isConnected() || Objects.equals(startPort.getParentSystem(), endPort.getParentSystem())) { JOptionPane.showMessageDialog(this.game, "Cannot create wire:\nInvalid connection attempt.", "Wiring Error", JOptionPane.WARNING_MESSAGE); return false; } int wireLength = (int) Math.round(startPort.getPosition().distance(endPort.getPosition())); if (wireLength <= 0) { java.lang.System.err.println("Wire creation failed: Zero length."); return false; } if (gameState.useWire(wireLength)) { try { Wire newWire = new Wire(startPort, endPort); synchronized (wires) { wires.add(newWire); } java.lang.System.out.println("Wire created: " + newWire.getId() + " Len: " + wireLength + ". Rem: " + gameState.getRemainingWireLength()); game.playSoundEffect("wire_connect"); updatePrediction(); return true; } catch (IllegalArgumentException | IllegalStateException e) { java.lang.System.err.println("Wire creation failed in constructor: " + e.getMessage()); gameState.returnWire(wireLength); JOptionPane.showMessageDialog(this.game, "Cannot create wire:\n" + e.getMessage(), "Wiring Error", JOptionPane.WARNING_MESSAGE); return false; } } else { JOptionPane.showMessageDialog(this.game, "Not enough wire! Need: " + wireLength + ", Have: " + gameState.getRemainingWireLength(), "Insufficient Wire", JOptionPane.WARNING_MESSAGE); game.playSoundEffect("error"); return false; } }
    ...
    public void deleteWireRequest(Wire wireToDelete) { if (wireToDelete == null || simulationStarted) { if(simulationStarted) { game.playSoundEffect("error"); java.lang.System.out.println("Cannot delete wire: Sim started."); } return; } boolean removed; synchronized (wires) { removed = wires.remove(wireToDelete); } if (removed) { int returnedLength = (int) Math.round(wireToDelete.getLength()); gameState.returnWire(returnedLength); wireToDelete.destroy(); game.playSoundEffect("wire_delete"); java.lang.System.out.println("Wire " + wireToDelete.getId() + " deleted. Ret: " + returnedLength + ". Rem: " + gameState.getRemainingWireLength()); updatePrediction(); repaint(); } else { java.lang.System.err.println("Warn: Failed to remove wire " + wireToDelete.getId()); } }
    ...
    public boolean isWireOccupied(Wire wire) { if (wire == null || !simulationStarted) return false; synchronized (packets) { List<Packet> currentPackets = new ArrayList<>(packets); for (Packet p : currentPackets) { if (p != null && !p.isMarkedForRemoval() && Objects.equals(p.getCurrentWire(), wire)) return true; } } return false; }
    ...
    private void updatePrediction() { if (simulationStarted) return; List<PacketSnapshot> newPrediction = predictNetworkStateDetailed(this.viewedTimeMs); synchronized(predictedPacketStates) { predictedPacketStates.clear(); predictedPacketStates.addAll(newPrediction); } repaint(); }
    private List<PacketSnapshot> predictNetworkStateDetailed(long targetTimeMs) { List<PacketSnapshot> snapshots = new ArrayList<>(); List<System.PredictedPacketInfo> potentialPacketsInfo = new ArrayList<>(); synchronized (systems) { List<System> currentSystems = new ArrayList<>(systems); for (System s : currentSystems) { if (s != null && s.isReferenceSystem() && s.hasOutputPorts()) potentialPacketsInfo.addAll(s.predictGeneratedPackets(targetTimeMs)); } } for (System.PredictedPacketInfo packetInfo : potentialPacketsInfo) { long currentSimTime = packetInfo.generationTimeMs; Port currentPort = packetInfo.sourcePort; if (currentSimTime > targetTimeMs) { snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape)); continue; } if (currentPort == null || currentPort.getPosition() == null) { java.lang.System.err.println("Pred Err: Invalid start port Pkt " + packetInfo.futurePacketId); snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape, null, PredictedPacketStatus.LOST, 0.0)); continue; } Point currentPosition = new Point(currentPort.getPosition());
        Wire currentWire = findWireFromPort(currentPort); double progress = 0.0; PredictedPacketStatus currentStatus = PredictedPacketStatus.ON_WIRE; int currentSystemId = -1; if (currentWire == null) { snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape, currentPosition, PredictedPacketStatus.LOST, 0.0)); continue; } NetworkEnums.PacketShape packetShapeEnum = packetInfo.shape; NetworkEnums.PortShape exitPortShape = currentPort.getShape(); NetworkEnums.PortShape requiredPortShapeForPacket = Port.getShapeEnum(packetShapeEnum); boolean compatibleExit = (requiredPortShapeForPacket != null && (exitPortShape == requiredPortShapeForPacket || exitPortShape == NetworkEnums.PortShape.ANY));
        double currentSpeed; boolean isAccelerating; double targetSpeed; if (packetShapeEnum == NetworkEnums.PacketShape.SQUARE) { currentSpeed = compatibleExit ? (Packet.BASE_SPEED_MAGNITUDE * Packet.SQUARE_COMPATIBLE_SPEED_FACTOR) : Packet.BASE_SPEED_MAGNITUDE; isAccelerating = false; targetSpeed = currentSpeed; } else if (packetShapeEnum == NetworkEnums.PacketShape.TRIANGLE) { currentSpeed = Packet.BASE_SPEED_MAGNITUDE; isAccelerating = !compatibleExit; targetSpeed = isAccelerating ? Packet.MAX_SPEED_MAGNITUDE : currentSpeed; } else { currentSpeed = Packet.BASE_SPEED_MAGNITUDE; isAccelerating = false; targetSpeed = currentSpeed; }
        ...
        while (currentSimTime < targetTimeMs) {
            ...
            if (isAccelerating) { currentSimSpeedPerMs = (currentSpeed + targetSpeed) / 2.0 / GAME_TICK_MS; } else { currentSimSpeedPerMs = currentSpeed / GAME_TICK_MS; }
            ...
            if (timeToFinishMs <= timeRemMs + PREDICTION_FLOAT_TOLERANCE) {
                currentSimTime += timeToFinishMs; progress = 1.0; Port endPort = currentWire.getEndPort(); if (endPort == null || endPort.getPosition() == null) { currentStatus = PredictedPacketStatus.LOST; currentPosition = (currentWire.getEndPort() != null && currentWire.getEndPort().getPosition() != null) ? currentWire.getEndPort().getPosition() : currentPosition; currentWire = null; break; } currentPosition.setLocation(endPort.getPosition()); System nextSystem = endPort.getParentSystem(); if (nextSystem == null) { currentStatus = PredictedPacketStatus.LOST; currentWire = null; break; } if (nextSystem.isReferenceSystem() && !nextSystem.hasOutputPorts()) {
                    boolean compat = false; NetworkEnums.PortShape req = Port.getShapeEnum(packetInfo.shape); if (req != null) { synchronized(nextSystem.getInputPorts()) { for (Port p : nextSystem.getInputPorts()) { if (p != null && (p.getShape() == req || p.getShape() == NetworkEnums.PortShape.ANY)) { compat = true; break; } } } } currentStatus = compat ? PredictedPacketStatus.DELIVERED : PredictedPacketStatus.LOST; currentWire = null; break; } else {
                    Port nextOut = findPredictedNextPort(packetInfo.shape, nextSystem); if (nextOut != null && nextOut.getPosition() != null) { Wire nextW = findWireFromPort(nextOut); if (nextW != null) {
                        ...
                        exitPortShape = currentPort.getShape(); requiredPortShapeForPacket = Port.getShapeEnum(packetShapeEnum); compatibleExit = (requiredPortShapeForPacket != null && (exitPortShape == requiredPortShapeForPacket || exitPortShape == NetworkEnums.PortShape.ANY)); if (packetShapeEnum == NetworkEnums.PacketShape.SQUARE) { currentSpeed = compatibleExit ? (Packet.BASE_SPEED_MAGNITUDE * Packet.SQUARE_COMPATIBLE_SPEED_FACTOR) : Packet.BASE_SPEED_MAGNITUDE; isAccelerating = false; targetSpeed = currentSpeed; } else if (packetShapeEnum == NetworkEnums.PacketShape.TRIANGLE) { currentSpeed = Packet.BASE_SPEED_MAGNITUDE; isAccelerating = !compatibleExit; targetSpeed = isAccelerating ? Packet.MAX_SPEED_MAGNITUDE : currentSpeed; } else { currentSpeed = Packet.BASE_SPEED_MAGNITUDE; isAccelerating = false; targetSpeed = currentSpeed; } } else { currentStatus = PredictedPacketStatus.STALLED_AT_NODE; currentPosition.setLocation(nextSystem.getPosition()); currentSystemId = nextSystem.getId(); currentWire = null; break; } } else { currentStatus = PredictedPacketStatus.STALLED_AT_NODE; currentPosition.setLocation(nextSystem.getPosition()); currentSystemId = nextSystem.getId(); currentWire = null; break; } } } else {
                double distToMove = currentSimSpeedPerMs * timeRemMs;
                if (wireLen > PREDICTION_FLOAT_TOLERANCE) { progress += (distToMove / wireLen); } progress = Math.max(0.0, Math.min(progress, 1.0)); Point newPos = currentWire.getPointAtProgress(progress); if (newPos != null) currentPosition = newPos; else java.lang.System.err.println("Pred Warn: Cannot get pos Pkt " + packetInfo.futurePacketId); currentSimTime = targetTimeMs; currentStatus = PredictedPacketStatus.ON_WIRE; break; } }
        if (currentStatus == PredictedPacketStatus.STALLED_AT_NODE) { snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape, currentPosition, currentStatus, currentSystemId)); } else { snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape, currentPosition, currentStatus, progress)); } } return snapshots; }
    private Port findPredictedNextPort(NetworkEnums.PacketShape packetShape, System node) { if (node == null || (node.isReferenceSystem() && !node.hasOutputPorts())) return null; if (node.isReferenceSystem() && node.hasOutputPorts()) { java.lang.System.err.println("Prediction Warning: Packet reached a Source system ("+node.getId()+") during prediction. Invalid route."); return null; } List<Port> compatiblePorts = new ArrayList<>(); List<Port> anyPorts = new ArrayList<>(); NetworkEnums.PortShape requiredShape = Port.getShapeEnum(packetShape); if (requiredShape == null) return null; synchronized(node.getOutputPorts()) { List<Port> currentOutputPorts = new ArrayList<>(node.getOutputPorts()); for (Port p : currentOutputPorts) { if (p != null && p.isConnected()) { if (p.getShape() == requiredShape) compatiblePorts.add(p); else if (p.getShape() == NetworkEnums.PortShape.ANY) anyPorts.add(p); } } } if (!compatiblePorts.isEmpty()) { Collections.shuffle(compatiblePorts, predictionRandom); return compatiblePorts.get(0); } if (!anyPorts.isEmpty()) { Collections.shuffle(anyPorts, predictionRandom); return anyPorts.get(0); } return null; }
