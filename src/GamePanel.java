...
public class GamePanel extends JPanel {
    ...
    private void detectAndHandleCollisions(List<Packet> packetSnapshot) {
        Set<Packet> lostInCollision = new HashSet<>();
    ...
    private void handleImpactWaveNoise(Point center, double maxNoise, List<Packet> snapshot, Packet ignore1, Packet ignore2) {
        ...
        for (Packet p : snapshot) {
            if (p == null || p.isMarkedForRemoval() || p.getCurrentSystem() != null || p.getCurrentWire() == null || p == ignore1 || p == ignore2) continue;
    ...
    private void checkEndConditions() {
        ...
        if (allSourcesFinishedGenerating && packetsOnWireEmpty && addBufferEmpty && removeBufferEmpty && queuesAreEmpty) {
            if (!levelComplete) {
                ...
                if (nextLevelIndex < gameState.getMaxLevels()) { gameState.unlockLevel(nextLevelIndex); }
    ...
    private void showEndLevelDialog(boolean success) {
        ...
        if (success) {
            if (nextLevelUnlocked) {
                ...
                java.lang.System.err.println("Warning: Level " + currentLevel + " complete, but next level "+nextLevelNumber+" is not unlocked."); 
                optionsList.add(menuOption);
            }
        } else {
        ...
        if (selectedOption.equals(menuOption)) {
            ...
            java.lang.System.err.println("Warning: Unknown option selected in end level dialog: " + selectedOption); 
            game.returnToMenu();
        }
    }
    ...
    public void packetLost(Packet packet) {
        if (packet != null && !packet.isMarkedForRemoval()) {
            ...
            if(packet.getCurrentWire() == null && packet.getCurrentSystem() == null && !lostInCollision.contains(packet)) reason = "Collision"; 
            else if (packet.getNoise() >= packet.getSize()) reason = "Noise";
            else reason = "Deviation/Route/Sink/Queue"; 
    ...
    private List<PacketSnapshot> predictNetworkStateDetailed(long targetTimeMs) {
        ...
        for (System.PredictedPacketInfo packetInfo : potentialPacketsInfo) { 
            long currentSimTime = packetInfo.generationTimeMs; Port currentPort = packetInfo.sourcePort;
            if (currentSimTime > targetTimeMs) { snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape)); continue; }
            if (currentPort == null || currentPort.getPosition() == null) { java.lang.System.err.println("Pred Err: Invalid start port Pkt " + packetInfo.futurePacketId); snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape, null, PredictedPacketStatus.LOST, 0.0)); continue; }
            Point currentPosition = new Point(currentPort.getPosition()); Wire currentWire = findWireFromPort(currentPort); double progress = 0.0; PredictedPacketStatus currentStatus = PredictedPacketStatus.ON_WIRE; int currentSystemId = -1;
            if (currentWire == null) { snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape, currentPosition, PredictedPacketStatus.LOST, 0.0)); continue; }
            while (currentSimTime < targetTimeMs) {
                if (currentWire == null) break;
                double timeRem = targetTimeMs - currentSimTime; double wireLen = currentWire.getLength(); if (wireLen < PREDICTION_FLOAT_TOLERANCE) progress = 1.0;
                double distRem = wireLen * (1.0 - progress); double speed = Packet.BASE_SPEED_MAGNITUDE; if (speed < PREDICTION_FLOAT_TOLERANCE) break;
                double timeFinish = (wireLen > PREDICTION_FLOAT_TOLERANCE) ? distRem / speed : 0.0;
                if (timeFinish <= timeRem + PREDICTION_FLOAT_TOLERANCE) {
                    currentSimTime += timeFinish; progress = 1.0; Port endPort = currentWire.getEndPort();
                    ...
                    if (nextSystem.isReferenceSystem() && !nextSystem.hasOutputPorts()) { boolean compat = false; NetworkEnums.PortShape req = Port.getShapeEnum(packetInfo.shape); if (req != null) { for (Port p : nextSystem.getInputPorts()) { if (p != null && (p.getShape() == req || p.getShape() == NetworkEnums.PortShape.ANY)) { compat = true; break; } } } currentStatus = compat ? PredictedPacketStatus.DELIVERED : PredictedPacketStatus.LOST; currentWire = null; break; }
                    else { Port nextOut = findPredictedNextPort(packetInfo.shape, nextSystem); if (nextOut != null) { Wire nextW = findWireFromPort(nextOut); if (nextW != null) { currentWire = nextW; currentPort = nextOut; currentPosition.setLocation(currentPort.getPosition()); progress = 0.0; currentStatus = PredictedPacketStatus.ON_WIRE; } else { currentStatus = PredictedPacketStatus.STALLED_AT_NODE; currentPosition.setLocation(nextSystem.getPosition()); currentSystemId = nextSystem.getId(); currentWire = null; break; } } else { currentStatus = PredictedPacketStatus.STALLED_AT_NODE; currentPosition.setLocation(nextSystem.getPosition()); currentSystemId = nextSystem.getId(); currentWire = null; break; } }
                } else { double distMove = speed * timeRem; if (wireLen > PREDICTION_FLOAT_TOLERANCE) progress += (distMove / wireLen); progress = Math.max(0.0, Math.min(progress, 1.0)); Point newPos = currentWire.getPointAtProgress(progress); if (newPos != null) currentPosition = newPos; else java.lang.System.err.println("Pred Warn: Cannot get pos Pkt " + packetInfo.futurePacketId); currentSimTime = targetTimeMs; currentStatus = PredictedPacketStatus.ON_WIRE; break; }
            }
            if (currentStatus == PredictedPacketStatus.STALLED_AT_NODE) snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape, currentPosition, currentStatus, currentSystemId)); else snapshots.add(new PacketSnapshot(packetInfo.futurePacketId, packetInfo.shape, currentPosition, currentStatus, progress));
        }
        return snapshots;
    }
    private Port findPredictedNextPort(NetworkEnums.PacketShape packetShape, System node) { 
        ...
        List<Port> compatiblePorts = new ArrayList<>(); List<Port> anyPorts = new ArrayList<>(); NetworkEnums.PortShape requiredShape = Port.getShapeEnum(packetShape); if (requiredShape == null) return null;
        for (Port p : node.getOutputPorts()) { if (p != null && p.isConnected()) { if (p.getShape() == requiredShape) compatiblePorts.add(p); else if (p.getShape() == NetworkEnums.PortShape.ANY) anyPorts.add(p); } }
