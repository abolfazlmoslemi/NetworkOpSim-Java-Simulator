...
public class KeyBindings {
    ...
    public void loadBindingsFromFile() {
        ...
        if (!file.exists()) {
            java.lang.System.out.println("Key bindings file not found ("+configFile+"). Using defaults and attempting to create one on next save.");
            return;
        }
        try (InputStream input = new FileInputStream(file)) {
            ...
            for (String keyName : properties.stringPropertyNames()) {
                try {
                    ...
                    java.lang.System.err.println("Warning: Unknown action or invalid keycode in " + configFile + ": " + keyName);
                }
            }
            java.lang.System.out.println("Key bindings loaded from " + configFile);
        } catch (IOException io) {
            java.lang.System.err.println("Error loading key bindings from " + configFile + ": " + io.getMessage() + ". Using defaults.");
            loadDefaultBindings();
        }
    }
