...
public class Packet {
    ...
    // NEW: Packet type and original type for state changes
    ...
    // Modified constructor to accept PacketType
    public Packet(NetworkEnums.PacketShape shape, double startX, double startY, NetworkEnums.PacketType type) {
        ...
        this.originalPacketType = type; // Store the initial type
        ...
        switch (shape) {
            case SQUARE:
                this.size = 2;
                this.baseCoinValue = (type == NetworkEnums.PacketType.SECRET) ? 5 : 1; // Secret packets are more valuable
                break;
            case TRIANGLE:
                this.size = 3;
                this.baseCoinValue = (type == NetworkEnums.PacketType.SECRET) ? 8 : 2;
                break;
            case CIRCLE: // Example for future
                this.size = 1;
    ...
    // Overloaded constructor for default NORMAL packets
    ...
    public void draw(Graphics2D g2d) {
        ...
        try {
            ...
            // --- DRAW OVERLAYS BASED ON PACKET TYPE ---
            ...
            switch (packetType) {
                ...
                case TROJAN:
                    ...
                    break;
                case SECRET:
                    g2d.setColor(new Color(255, 100, 255, 200));
                    g2d.setStroke(new BasicStroke(1.0f));
                    if (transformed) { // Draw 'S' in center
                        g2d.setFont(new Font("Arial", Font.BOLD, 9));
                        g2d.drawString("S", -3, 3);
                    } else {
                        g2d.setFont(new Font("Arial", Font.BOLD, 9));
                        g2d.drawString("S", drawX + 2, drawY + 9);
                    }
                    break;
                case MESSENGER:
                    g2d.setColor(new Color(255, 255, 100, 255));
                    g2d.setStroke(new BasicStroke(1.0f));
                    if (transformed) { // Draw checkmark
                        g2d.setStroke(new BasicStroke(2.0f));
                        g2d.drawLine(-3, 0, -1, 3);
                        g2d.drawLine(-1, 3, 3, -2);
                    } else {
                        g2d.setStroke(new BasicStroke(2.0f));
                        g2d.drawLine(drawX + 2, drawY + 5, drawX + 4, drawY + 8);
                        g2d.drawLine(drawX + 4, drawY + 8, drawX + 8, drawY + 3);
                    }
                    break;
            }
            g2d.setStroke(defaultStroke);

            // --- DRAW NOISE OVERLAY AND TEXT ---
            if (noise > 0.05) {
    ...
    // --- NEW Getters and Setters for PacketType ---
    ...
    public void setPacketType(NetworkEnums.PacketType packetType) {
        if(this.packetType != packetType){
            this.originalPacketType = this.packetType; // Save previous type before changing
    ...
    // --- Other methods (update, collidesWith, etc.) remain unchanged ---
    // ... [ The rest of the Packet.java code from the prompt remains here, unchanged ] ...
    // Note: I'm omitting the rest of the file for brevity, as it was provided in the prompt
    // and this section only details the changes. The full file should include all original methods.
    ...
    public void setWire(Wire wire, boolean compatiblePortExit) {
        ...
        if (this.finalStatusForPrediction == PredictedPacketStatus.QUEUED ||
                ...
                this.finalStatusForPrediction == PredictedPacketStatus.LOST) {
    ...
    public void setVisualOffsetDirectionFromForce(Point2D.Double forceDirection) {
        if (currentWire == null) {
            if (this.visualOffsetDirection == null) this.visualOffsetDirection = new Point2D.Double(0,1);
            return;
        }
    ...
    public Point getPosition() {
        ...
        return new Point( (int)Math.round(idealPosition.x + offset.x), (int)Math.round(idealPosition.y + offset.y) );
    }
    public Point2D.Double getPositionDouble() {
        if (idealPosition == null) return new Point2D.Double();
        Point2D.Double offset = calculateCurrentVisualOffset();
        return new Point2D.Double( idealPosition.x + offset.x, idealPosition.y + offset.y );
    }
    ...
    public void resetNoise() {
        if (!markedForRemoval && this.noise > 0) {
            this.noise = 0.0;
            this.visualOffsetDirection = null;
        }
    }
    public boolean collidesWith(Packet other) {
        ...
        if (this.currentSystem != null || other.currentSystem != null || this.markedForRemoval || other.markedForRemoval) return false;
    ...
    @Override public String toString() {
        ...
        return String.format("Packet{ID:%d, SHP:%s, TYP: %s, SZ:%d, N:%.1f/%d, V:%d, Ideal:(%s) Vel:(%s) Accel:%b FinalPred:%s St:%s}",
                id, shape, packetType, size, noise, this.size, baseCoinValue, idealPosStr, velStr, isAccelerating,
