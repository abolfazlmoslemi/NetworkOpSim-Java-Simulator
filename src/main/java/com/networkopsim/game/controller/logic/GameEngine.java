...
public class GameEngine {
    ...
    public void startSimulation() {
        ...
        Packet.resetGlobalId();
        com.networkopsim.game.model.core.System.resetGlobalRandomSeed(gamePanel.getPredictionSeed());
        for (com.networkopsim.game.model.core.System s : systems) { s.resetForNewRun(); }
    ...
    public void gameTick(long tickDurationMs) {
        ...
        runSimulationTickLogic(false, simulationTimeElapsedMs, gamePanel.isAtarActive(), gamePanel.isAiryamanActive(), gamePanel.isSpeedLimiterActive());
    }

    public void loadFromSaveData(GameStateManager.SaveData saveData) {
        logger.info("GameEngine: Loading state from save data. Time: {}ms", saveData.simulationTimeElapsedMs);
        stopAndCleanupLevel();
        this.gameState = saveData.gameState;
        this.simulationTimeElapsedMs = saveData.simulationTimeElapsedMs;
        this.systems = new ArrayList<>(saveData.systems);
        this.wires = new ArrayList<>(saveData.wires);
        this.packets = new ArrayList<>(saveData.packets);
    ...
    public List<Packet> getPacketsForRendering() { return packets.stream().filter(p -> p != null && p.getCurrentSystem() == null).collect(Collectors.toList()); }
