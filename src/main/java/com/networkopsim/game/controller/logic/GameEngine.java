...
public class GameEngine {
    ...
    public void packetLostInternal(Packet packet, boolean isPredictionRun) { if (packet != null && !packet.isMarkedForRemoval()) { if (!isPredictionRun) logger.warn("Lost: {}", packet); packet.markForRemoval(); packet.setFinalStatusForPrediction(PredictedPacketStatus.LOST); packetsToRemove.add(packet); if (packet.getBulkParentId() != -1) { resolveBulkPart(packet.getBulkParentId(), packet.getId()); } if (isPredictionRun) { gamePanel.getPredictionContext().registerLostPacket(packet); } else { gameState.increasePacketLoss(packet); if (!game.isMuted()) game.playSoundEffect("packet_loss"); } } }
