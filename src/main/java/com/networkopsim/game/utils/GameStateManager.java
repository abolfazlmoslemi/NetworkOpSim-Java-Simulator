...
public class GameStateManager {
    ...
    public static boolean saveGameState(GamePanel gamePanel) {
        ...
        GameEngine gameEngine = gamePanel.getGameEngine(); // Get the engine from the panel
        try (ObjectOutputStream oos = new ObjectOutputStream(new GZIPOutputStream(new FileOutputStream(SAVE_FILE_NAME)))) {
            // Use internal deepCopy method for self-containment
            List<System> systemsCopy = deepCopy(new ArrayList<>(gameEngine.getSystems()));
            List<Wire> wiresCopy = deepCopy(new ArrayList<>(gameEngine.getWires()));
            List<Packet> packetsCopy = deepCopy(new ArrayList<>(gameEngine.getPackets()));
            GameState gameStateCopy = deepCopy(gameEngine.getGameState());
