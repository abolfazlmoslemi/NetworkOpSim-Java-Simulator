// FILE: /NetworkOpSim-Multiplayer/NetworkOpSim-Core/src/main/java/com/networkopsim/core/game/model/core/Packet.java
// ================================================================================
...
public class Packet implements Serializable {
    ...
    // --- NEW FIELDS FOR TROJAN OWNERSHIP SWAP ---
    private int originalOwnerId = -1;
    private boolean isConvertedTrojan = false;

    public static void resetGlobalId() { nextPacketId = 0; }
    public Packet(NetworkEnums.PacketShape shape, double startX, double startY, NetworkEnums.PacketType type) { this.id = nextPacketId++; this.shape = Objects.requireNonNull(shape, "Packet shape cannot be null"); this.packetType = Objects.requireNonNull(type, "Packet type cannot be null"); this.originalPacketType = type; this.idealPosition = new Point2D.Double(startX, startY); this.velocity = new Point2D.Double(0, 0); switch (type) { case SECRET: this.size = 4; this.baseCoinValue = 3; break; case BULK: this.size = 8; this.baseCoinValue = 8; break; case WOBBLE: this.size = 10; this.baseCoinValue = 10; break; case MESSENGER: this.size = 1; this.baseCoinValue = 1; break; default: if (shape == NetworkEnums.PacketShape.SQUARE) { this.size = 2; this.baseCoinValue = 2; } else if (shape == NetworkEnums.PacketShape.TRIANGLE) { this.size = 3; this.baseCoinValue = 3; } else { this.size = 1; this.baseCoinValue = 1; } break; } this.originalSize = this.size; this.originalBaseCoinValue = this.baseCoinValue; this.initialOffsetSidePreference = this.id % 2; this.momentOfInertia = 0.5f * this.size * this.getDrawSize(); this.positionHistory = new LinkedList<>(); updateHitbox(); }

    // Overloaded method for backward compatibility / simplicity in other calls
    public void update(GameEngine gameEngine, boolean isAiryamanActive, boolean isSpeedLimiterActive, boolean isPredictionRun) {
        update(gameEngine, isAiryamanActive, isSpeedLimiterActive, isPredictionRun, 1.0);
    }

    public void update(GameEngine gameEngine, boolean isAiryamanActive, boolean isSpeedLimiterActive, boolean isPredictionRun, double speedModifier) {
        if (markedForRemoval || currentSystem != null || isGhost) return;
        if (currentWire == null) {
            gameEngine.packetLostInternal(this, isPredictionRun);
            return;
        }
        timeOnCurrentWireMs += 16; if (timeOnCurrentWireMs > WIRE_TIMEOUT_MS) { gameEngine.packetLostInternal(this, isPredictionRun); return; }
        ...
        if (this.packetType == NetworkEnums.PacketType.SECRET) { if (isUpgradedSecret) { handleUpgradedSecretMovement(gameEngine); } else { System destSystem = currentWire.getEndPort().getParentSystem(); targetSpeedMagnitude = (destSystem != null && destSystem.getQueueSize() > 0) ? BASE_SPEED_MAGNITUDE * SECRET_PACKET_SLOW_SPEED_FACTOR : BASE_SPEED_MAGNITUDE; currentSpeedMagnitude = targetSpeedMagnitude; isAccelerating = false; } }
        ...
        double finalSpeed = currentSpeedMagnitude * speedModifier; // Apply global speed modifier

        double wireLength = currentWire.getLength();
        if (wireLength > 1e-6) { progressOnWire += (isReversing ? -1 : 1) * finalSpeed / wireLength; } else { progressOnWire = isReversing ? 0.0 : 1.0; }

        progressOnWire = Math.max(0.0, Math.min(1.0, progressOnWire));
        updateIdealPositionAndVelocity(speedModifier);
        updateHitbox();
        Point2D.Double currentVisualPos = getVisualPosition();
        if (currentVisualPos != null) { positionHistory.add(currentVisualPos); if (positionHistory.size() > POSITION_HISTORY_SIZE) { ((LinkedList<Point2D.Double>) positionHistory).removeFirst(); } }
    ...
    private void handleArrival(Port destinationPort, GameEngine gameEngine, boolean isPredictionRun) {
        ...
        if (this.isVolumetric() && !isPredictionRun) {
    ...
    public void reverseDirection(GameEngine gameEngine) {
    ...
    private void updateIdealPositionAndVelocity() {
        updateIdealPositionAndVelocity(1.0);
    }

    private void updateIdealPositionAndVelocity(double speedModifier) {
        if (currentWire == null) return;
        Wire.PathInfo pathInfo = currentWire.getPathInfoAtProgress(progressOnWire);
        if (pathInfo != null) {
            this.idealPosition = pathInfo.position;
            double finalSpeed = currentSpeedMagnitude * speedModifier;
            double dx = pathInfo.direction.x * finalSpeed;
            double dy = pathInfo.direction.y * finalSpeed;
    ...
    /**
     * Reverts a converted trojan back to its original owner and type.
     * This method combines reverting the packet type (like from PROTECTED) and also reverting the owner if it was a converted trojan.
     */
    public void revertOwnerAndType() {
        // First, revert the packet type (e.g., from PROTECTED to NORMAL)
        revertToOriginalType();

        // Then, if it was a trojan that had its ownership swapped, revert the owner.
        if (this.isConvertedTrojan && this.originalOwnerId != -1) {
            this.setOwnerId(this.originalOwnerId);
            this.isConvertedTrojan = false;
            this.originalOwnerId = -1;
        }
    }

    public void transformToMessenger() { this.packetType = NetworkEnums.PacketType.MESSENGER; this.size = 1; this.baseCoinValue = 1; }
    ...
    // --- NEW GETTERS/SETTERS FOR TROJAN SWAP ---
    public boolean isConvertedTrojan() {
        return isConvertedTrojan;
    }

    public void setConvertedTrojan(boolean convertedTrojan) {
        isConvertedTrojan = convertedTrojan;
    }

    public void setOriginalOwnerId(int ownerId) {
